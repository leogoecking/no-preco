...
const mockTransaction = jest.fn((ops: unknown[]) => Promise.all(ops as Promise<unknown>[]));
...
describe('PrecoRepository.salvarLote', () => {
  ...
  it('insere quando não existe registro anterior para (produto, cnpj)', async () => {
    ...
    expect(mockUpdateMany).not.toHaveBeenCalled();
    const chamada = mockCreateMany.mock.calls[0]![0] as {
      data: { produto: string; fonte: string }[];
    };
    expect(chamada.data[0]!.produto).toBe('arroz 5kg');
    expect(chamada.data[0]!.fonte).toBe('api');
  });

  it('atualiza apenas dataColeta quando o preço não mudou', async () => {
    mockQueryRaw.mockResolvedValue([
      { id: 42, produto: 'arroz 5kg', cnpj: '00.000.000/0001-00', preco: new Prisma.Decimal(25.9) },
    ]);
  ...
  it('mistura updates e inserts no mesmo lote em única transação', async () => {
    mockQueryRaw.mockResolvedValue([
      { id: 10, produto: 'arroz 5kg', cnpj: '00.000.000/0001-00', preco: new Prisma.Decimal(25.9) },
    ]);
    mockUpdateMany.mockResolvedValue({ count: 1 });
    mockCreateMany.mockResolvedValue({ count: 1 });
    ...
    const result = await repo.salvarLote(
      [
        produtoPreco({ preco: 25.9 }), // preço igual → update
        produtoPreco({ nome: 'Feijão', cnpj: '11.111.111/0001-11', preco: 10 }), // novo → create
      ],
      'api',
    );
