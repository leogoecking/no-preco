...
// ─────────────────────────────────────────────
// GET /analise/carrinho
// ?municipio=Teixeira+de+Freitas&itens=arroz:2,feijao:1,oleo:1
// ─────────────────────────────────────────────

export async function analisarGet(req: Request, res: Response): Promise<void> {
  const municipio =
    typeof req.query['municipio'] === 'string' ? req.query['municipio'].trim() : undefined;
  const itensParam = typeof req.query['itens'] === 'string' ? req.query['itens'].trim() : '';

  if (!itensParam) {
    res.status(400).json({
      erro: 'Parâmetro "itens" obrigatório.',
      exemplo: '/analise/carrinho?municipio=Teixeira+de+Freitas&itens=arroz:2,feijao:1,oleo:1',
    });
    return;
  }
